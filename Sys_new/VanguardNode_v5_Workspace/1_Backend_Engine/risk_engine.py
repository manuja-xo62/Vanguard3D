...
def load_config(config_path: str=str(CONFIG_PATH)) -> Dict[str, Any]:
    ...
    if not cfg_file.exists():
        return {'severity_weights': {'LOW': 1, 'MEDIUM': 3, 'HIGH': 7, 'CRITICAL': 15}, 'exposure_multiplier': {'internet_facing': 2.0, 'internal_only': 1.0}, 'blast_radius_weights': {'iam_role': 3.0, 'security_group': 2.0, 'storage_bucket': 2.0, 'default': 1.0}, 'criticality_weights': {'prod/**': 3.0, 'staging/**': 1.5, '**': 1.0}, 'remediation_templates': {'CKV_AWS_20': {'search_pattern': 'acl\\s*=\\s*"public-read"', 'patch_text': 'acl    = "private" # [VANGUARD NANO-PATCH APPLIED]', 'type': 'replace'}, 'CKV_AWS_21': {'search_pattern': '(resource\\s+"aws_s3_bucket"\\s+"[^"]+"\\s*\\{)', 'patch_text': '\\1' + '\n  versioning {\n    enabled = true\n  } # [VANGUARD NANO-PATCH APPLIED]', 'type': 'replace'}, 'CKV_DOCKER_3': {'action': 'insert_user', 'patch_text': 'USER vanguard_svc # [VANGUARD NANO-PATCH APPLIED]\n', 'type': 'insert'}}}
    try:
...
def get_file_criticality(file_path: str, criticality_weights: Dict[str, float]) -> float:
    ...
    sorted_patterns = sorted(criticality_weights.items(), key=lambda item: (item[0] == '**', item[0] == '*', -len(item[0])))
...
def calculate_risk(findings: Union[List[Dict[str, Any]], Dict[str, Any]], config_path: str=str(CONFIG_PATH)) -> Dict[str, Any]:
    ...
    for file_path, file_findings in files_map.items():
        ...
        file_results.append({'file_path': file_path, 'FilePath': file_path, 'findings_count': len(file_findings), 'R_file': round(r_file, 2), 'r_file': round(r_file, 2), 'file_criticality': w_crit, 'findings': processed_findings})
    ...
    return {'R_global': r_global, 'r_global': r_global, 'files': file_results}
    ...
    def on_modified(self, event):
        ...
        if any((src_str.endswith(ext) for ext in ['.tf', '.yaml', '.yml', 'Dockerfile'])):
            ...
            try:
                ...
                record_scan(scan_id=f'wd_{int(time.time())}', target_dir=self.target_dir, mode='watchdog', source='watchdog', r_global=risk_data['R_global'], files_data=risk_data['files'])
